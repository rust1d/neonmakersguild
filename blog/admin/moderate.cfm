...
<cfif structKeyExists(url, "approve")>
  ...
  <cfset entry = application.blog.getEntry(c.bco_benid)>
...
#application.utils.ParagraphFormat2(htmlEditFormat(c.comment))#
  ...
  <cfset application.blog.notifyEntry(entryid=c.bco_benid, message=trim(email), subject=subject, from=c.email, noadmin=true, commentid=c.id, html=true)>

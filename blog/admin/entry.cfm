...
<cftry>
  <cfif url.id neq 0>
    ...
    <cfif len(entry.ben_morebody)>
      <cfset entry.body = entry.body & "<more/>" & entry.ben_morebody>
    </cfif>
  ...
  <cfelse>
    ...
    <cfparam name="form.ben_released" default="#application.blog.isBlogAuthorized('ReleaseEntries')#">
...
<cfelseif isDefined("form.manualben_attachment") and len(trim(form.manualben_attachment))>
  ...
  <cfif fileExists(destination & "/" & form.manualben_attachment)>
    ...
    <cfif not isDefined("form.oldben_mimetype")>
      <cfset form.oldben_mimetype = "application/unknown">
    </cfif>
  </cfif>
</cfif>

<cfif not isNumeric(form.oldben_filesize)>
  <cfset form.oldben_filesize = 0>
</cfif>
...
<cfif isDefined("form.save")>
  ...
  <cfif not arrayLen(errors)>
    ...
    <cfif isDefined("variables.entry")>
      <!--- Begin: Shane Zehnder | ben_released posts should have the post date of when they're ben_released --->
      <cfif (form.ben_released eq "true") and (entry.ben_released eq "false") and (dateCompare(dateAdd("h", application.blog.getProperty("offset"), form.posted), now()) lt 0)>
...
<cfmodule template="../tags/adminlayout.cfm" title="Entry Editor">
  ...
  <cfif not structKeyExists(form, "preview")>
    ...
    <div class="ctrlHolder">
      ...
      <cfif allCats.recordCount><select name="categories" id="categories" multiple="multiple" size="4">
        <cfloop query="allCats">
        <option value="#bca_bcaid#" <cfif isDefined("form.categories") and listFind(form.categories,bca_bcaid)>selected</cfif>>#bca_category#</option>
        </cfloop>
        </select>
      </cfif>
    </div>
    ...
    <div id="additional">
      ...
      <div class="ctrlHolder">
        <label for="ben_allowcomments">Allow Comments: </label>
        <select name="ben_allowcomments" id="ben_allowcomments">
        <option value="true" <cfif form.ben_allowcomments is "true">selected</cfif>>Yes</option>
        <option value="false" <cfif form.ben_allowcomments is "false">selected</cfif>>No</option>
        </select>
      </div>
      <div class="ctrlHolder">
      ...
      <div class="ctrlHolder">
        <label for="ben_released">ben_released: </label>
        <cfif application.blog.isBlogAuthorized('ReleaseEntries')>
          <select name="ben_released" id="ben_released">
          <option value="true" <cfif form.ben_released is "true">selected</cfif>>Yes</option>
          <option value="false" <cfif form.ben_released is "false">selected</cfif>>No</option>
          </select>
        <cfelse>
          #yesNoFormat(form.ben_released)#
        </cfif>
      </div>
      <div class="ctrlHolder">
  ...
  <cfelse>
    ...
    <a href="#application.blog.makeCategoryLink(listGetAt(form.categories,x))#">#application.blog.getCategory(listGetAt(form.categories,x)).bca_category#</a><cfif x is not listLen(form.categories)>,</cfif>
    </cfloop>
    </div>

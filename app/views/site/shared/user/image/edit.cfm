<cfscript>
  ...
  locals.mImage = locals.mBlog.image_find_or_create(router.decode('uiid'));

  if (form.keyExists('btnSubmit')) {
    locals.mImage.set(form);
    if (locals.mImage.safe_save()) {
      flash.success('Your image was saved.');
      router.redirect('#locals.dest#/image/list');
    } else {
      flash.error('An error occurred while uploading. Please try again or contact #session.site.mailto_site()#.');
      router.redirenc('#locals.dest#/image/edit');
    }
  } else if (form.keyExists('btnDelete')) {
    locals.mImage.destroy();
    flash.success('Your image was deleted.');
    router.redirect('#locals.dest#/image/list');
  }

  locals.mode = locals.mImage.new_record() ? 'Add' : 'Edit';
</cfscript>
  ...
  <form role='form' method='post' enctype='multipart/form-data'>
    <div class='card'>
      <h5 class='card-header bg-nmg align-items-center'>
        #locals.mode#
        <cfif locals.mode is 'add'>
          Image
        <cfelse>
          #locals.mImage.filename()#
        </cfif>
      </h5>
      <div class='card-body'>
        <cfif locals.mode is 'add'>
          <form method='post' enctype='multipart/form-data'>
            <div class='row'>
              <div class='col-md-6 col-12'>
                <div class='row g-3'>
                  <div class='col-12'>
                    Images will be resized to a max height/width of #locals.mImage.size_image()#px and a thumbnail will be created for previews.
                  </div>
                  ...
                  <div class='col-12 text-center'>
                    ...
                    <a href='#router.href('#locals.dest#/image/list')#' class='btn btn-wide btn-nmg-cancel'>Cancel</a>
                  </div>
                </div>
              </div>
          ...
          <div class='row g-3'>
            <div class='col-md-auto'>
              <div class='position-relative d-inline-block'>
                <img id='thumbnail_src' src='#locals.mImage.thumbnail_src()#' class='img-thumbnail' />
                <button type='button' id='btnOpen' data-uiid='#locals.mImage.encoded_key()#' title='change thumbnail' class='btn btn-icon bottom-right btn-nmg btn-outline-dark'>
                  <i class='fa-solid fa-photo-film'></i>
                </button>
              </div>
              <div class='bg-nmg mt-3 fs-6'>
                <div class='border-bottom py-1 px-2'>
                  #locals.mImage.dimensions()#
                </div>
                <div class='border-bottom py-1 px-2'>
                  #locals.mImage.size_mb()#
                </div>
                <div class='border-bottom py-1 px-2'>
                  <span class='clipable' data-clip='#locals.mImage.image_src()#'>
                    <i class='fa-solid fa-fw fa-copy'></i> Image Link (#locals.mImage.dimensions()#)
                  </span>
                </div>
                <div class='py-1 px-2'>
                  <span class='clipable' data-clip='#locals.mImage.thumbnail_src()#'>
              ...
              <div class='mt-3'>
                ...
                <div class='input-group input-group-sm'>
                  <input type='text' class='form-control' name='ui_filename' value='#locals.mImage.filename()#' />
                  <button type='submit' name='btnSubmit' class='btn btn-sm btn-nmg'>go</button>
                </div>
              </div>
              <div class='mt-3 text-center'>
                <hr>
                <a href='#router.href('#locals.dest#/image/list')#' class='btn btn-wide btn-nmg'>Done</a>
              </div>
            </div>
            <div class='col-md'>
              <div class='position-relative d-inline-block'>
                <img src='#locals.mImage.image_src()#' class='img-thumbnail img-fluid clipable' data-clip='#locals.mImage.image_src()#' />
